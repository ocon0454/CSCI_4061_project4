...
void init(int port) {
   ...
   {
     printf("init(): Failed to create a socket\n");
     exit(-1);
   }
...
int accept_connection(void) {
  ...
  //  if ((newsock = accept(master_fd, (struct sockaddr*)&new_recv_addr, (socklen_t*) &addr_len)) == -1)
  //  {
  //   printf("Error accepting connection"); 
  //  } 
   newsock = accept(master_fd, (struct sockaddr*)&new_recv_addr, (socklen_t*)&addr_len);
